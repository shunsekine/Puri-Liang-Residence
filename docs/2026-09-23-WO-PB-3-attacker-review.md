...
最終更新日時: 2026-09-24（F1〜F3・F5・F6 の状態） / 更新エージェント: Claude (Opus 5.5)
...
| F3 | 中 | Sheets の数式インジェクション（`=HYPERLINK(...)`・`=IMPORTXML(...)`） | `SpreadsheetService.ts:100` `appendRow` に `name`・`remarks`・`roomType` を生で渡す | 先頭が `= + - @ \t` の値に `'` を前置（WhatsApp テキスト列も） | **コード対応済み・本番未反映**（WO-PB-3F）: `appendInquiry` が記帳する行の文字列すべてに適用（CR も対象） |
...
| F5 | 中 | レート制限・サイズ上限なし（GAS の実行枠と Sheets の行を消費される） | `route.ts` | F1 の対策で兼ねる | **コード対応済み・本番未反映**（WO-PB-3F）: 本文 16 KB まで（超えたら 413）。件数は F1 の自動送信の上限で止める（IP 単位の制限は入れない） |
| F6 | 低 | エラーメッセージの漏れ（`error.message` をそのまま返す） | `route.ts`・`Main.ts` の catch | 固定文言を返し、詳細はログのみ | **コード対応済み・本番未反映**（WO-PB-3F）: route のエラーは `{ success: false }` だけ（GAS の応答も透過しない）。`doPost` は `error: 'internal'` |
...
**次の一手**: F3・F5・F6 の本番反映（`docs/2026-09-24-WO-PB-3F-reserve-abuse.md` §3。F1・F2 は 2026-09-24 に反映済み）。F4・F7 は WO-PB-3F の範囲外（同 WO「範囲外の気づき」）。
