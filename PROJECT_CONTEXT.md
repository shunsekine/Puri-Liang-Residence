...
- **GAS のコードとメールの文面はリポジトリが正本**: エディタで直接直したら `gas-booking-automation/src` にも反映する。文面は `src/Templates.ts`（シートの旧 Templates タブは「Templates（旧・未使用）」で、GAS は読まない）。
- **GAS 失敗通知**は件名でなく本文の表で判定する（`gas-booking-automation/README.md`「失敗通知メールの読み方」）。
- **本番の GAS プロジェクト**は `1dEFqku0rLFM34SWz-8Sn7cjCX_aYCQf6gbKTmI79YTrL1349CXIi1lFn`（予約管理表_v1.0 に紐づく唯一のプロジェクト「無題のプロジェクト」）。エディタは直接 URL（`script.google.com/u/<番号>/home/projects/<ID>/edit`）で開く（Chrome で puriliang が既定以外のアカウントだと、Sheets の「拡張機能 → Apps Script」は「unable to open the file」になる）。
- **`Inquiries` の空行は意図したもの**: テスト行は行を消さず B〜S 列だけ消し、A 列の ID は残す（ID は `getLastRow` 採番。最終行の A 列まで消すと使用済みの ID が再利用される＝2026-09-25 に実際に起きた）。
- **VM 共用**: 他エージェントのプロセス・作業を予告なく止めない・上書きしない。

### デプロイ / 環境

- Vercel（shun-projects-workspace / owner puriliangresidence.bali@gmail.com / Hobby / リポ **Public**）。main へのマージ＝本番反映（既定ではユーザーがマージ。エージェントの代行はその都度の許可で、PR の Vercel チェックが通ってから）。プレビューはブランチごと（Deployment Protection 有効）。
...
1. （任意）2026-10-09 頃に Search Console の URL 検査で、2026-09-25 に再インデックスを申請した 15 件（`/privacy`・`/faq`・`/rooms`・`/reserve`・トップ × 3 言語）が「登録されています」になったか確認する。サイトマップ（21 件）も同日に再送信済み。
