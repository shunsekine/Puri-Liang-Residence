...
最終更新: 2026-09-25 / Claude (claude-opus-5-5) / 基準 SHA `db44c47`（main）
...
- **本番の GAS プロジェクト**は `1dEFqku0rLFM34SWz-8Sn7cjCX_aYCQf6gbKTmI79YTrL1349CXIi1lFn`（予約管理表_v1.0 に紐づく「無題のプロジェクト」）。同じシートに紐づく 2026-07-25 作成の同名プロジェクト（`1leeIINn…`）は未使用（2026-09-25 確認: 中身は空の `myFunction` だけ・トリガー 0・デプロイなし）。一覧の操作は「完全に削除」しかない（ゴミ箱に入らない）ので、消すならユーザーが行う。シートの旧 Templates タブは「Templates（旧・未使用）」に改名済み（GAS は読まない）。
...
最終更新: 2026-09-25 / Claude (claude-opus-5-5) / 基準 SHA `db44c47`
...
1. **GAS の新バージョンのデプロイ（ユーザー・puriliang）**: `db44c47` のコードは 2026-09-25 に貼り付け済み（8 ファイルとも再読み込み後のハッシュが build-gs.sh の出力と一致。直後の `processAutoReplies` も完了）。doPost の変更（F3・F6・秘密の空白除去）は「デプロイを管理 → 鉛筆 → 新バージョン」の後に有効になる。
