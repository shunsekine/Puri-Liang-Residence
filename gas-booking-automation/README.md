...
* **テンプレートの言語**: 一次返信・最終回答とも `Templates` の `${種類}_${言語}` を使い、無ければ `${種類}_en` で代用する（件名か本文が空の行は「無い」扱い）。代用したときは担当者に伝える（一次返信: 担当者通知メールに 1 行、最終回答: ステータスのセルのメモに日付付きで 1 行）。どちらも無ければ黙って止めない: 一次返信は行を `エラー` にして失敗通知、最終回答は下書きを作らずステータスのセルに理由をメモしてエラーにする
...
1. **変換**: 変更したファイルから `import` 行を消し、`export ` を外してから `tsc --target ES2019 --module none` で JS にする（全ファイルが 1 つのグローバル空間に並ぶ形。自動化は WO-PB-3F 段階 A）。
2. **貼り付け**: エディタの同名ファイルに貼る。リポジトリで増えたファイルはエディタにも足す。`Types.ts` は型だけなので貼らなくてよい。
3. **デプロイ**: `doPost`（Webhook）に関わる変更は「デプロイを管理 → 鉛筆（編集）→ バージョン: 新バージョン → デプロイ」。**「新しいデプロイ」は別 URL を作る**ので使わない。定期トリガー・`onEdit` から呼ばれる処理は、保存した最新のコードで動く。
...
文面はリポジトリの外にある。料金・支払い・キャンセル・電気・対応言語に触れる文面は、`PROJECT_CONTEXT.md`「事業ルール」とサイトの文言に揃える（2026-09 に `Available_*` のキャンセル規定が古いまま残っていた）。
...
* B列: `Subject` (メール件名)
* C列: `Body` (メール本文。使えるプレースホルダーは `{ID}` `{Name}` `{CheckIn}` `{CheckOut}` `{RoomType}` `{Guests}`（`EmailService.replacePlaceholders`）)
* 最終回答（`Available` `Full` `AcceptWaiting`）は下書きになり、件名は `Re: {Subject} ({ID})` になる。セルに複数行の本文を貼るときは、セルをダブルクリックして編集状態にしてから貼る（そのまま貼ると行が分かれる）
